...
public class ProductController {
    ...
    public ResponseEntity<Void> ajouterProduit(@Valid @RequestBody Product product)
            throws ProduitGratuitException{
        if(product.getPrix() == 0)
            throw new ProduitGratuitException("Il est interdit de creer un produit gratuit.");
    ...
    public Map<Product,Integer> calculerMargeProduits(){
        ...
        for (Product p : produits)
        {
            ...
            margesProduits.put(p, marge);
        }
        return margesProduits;
    }

    @GetMapping(value="/ProduitsSorted")
